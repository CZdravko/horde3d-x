...
void CrowdSim::init() {
	...
	for (unsigned int i = 0; i < 5; ++i) {
		...
		// Add lights to scene
//		l.node = h3dAddNodes(H3DRootNode, lightRes);
//
//		// Characters start in a circle formation
//		l.px = sinf((i / 5.0f) * 6.28f) * 10.0f;
//		l.pz = cosf((i / 5.0f) * 6.28f) * 10.0f;
//
//		chooseDestination(l);
//
//		h3dSetNodeTransform(l.node, l.px, 0.02f, l.pz, 0, 0, 0, 1, 1, 1);
//
//		_lights.push_back(l);
	}
}
...
void CrowdSim::update(float fps) {
	...
	for (unsigned int i = 0; i < _particles.size(); ++i) {
		...
		float dist = sqrtf(
				(p.dx - p.px) * (p.dx - p.px) + (p.dz - p.pz) * (p.dz - p.pz));
		...
		if (dist > 3.0f) {
			...
			for (unsigned int j = 0; j < _particles.size(); ++j) {
				...
				float dist2 = sqrtf(
						(p.px - p2.px) * (p.px - p2.px)
								+ (p.pz - p2.pz) * (p.pz - p2.pz));
				float strength = 0;
